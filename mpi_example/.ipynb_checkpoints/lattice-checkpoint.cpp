...
lattice_2d_ysymm::lattice_2d_ysymm(int L, int Nt, double dt, double J, double Eint, double mu, int size)
{
    assert(-1<=Nt);
    assert(-1<=Nt);
    Nt_=Nt;
...
void lattice_2d_ysymm::efield_to_afield(int Nt, double dt, double Eint)
{
    Vector2D efield(Eint,0);
    A_.resize(Nt + 1);
